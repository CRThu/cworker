...
- 🚀 **远端文件五件套**：
  - `cp`：支持文件与目录的 本地 $\leftrightarrow$ 远端，以及远端到远端的内存管道流式直传（`-r` 递归、`-j` 并发传输、空目录守恒、自动递归创建父级目录）；全链路单遍流式 SHA-256 强一致性核验与脏数据自动回滚；自适应平滑进度条（TTY 环境实时刷新，Non-TTY/Agent 环境单行摘要纯净输出）。
  - `cat`：直接在本地终端打印远端文本，免除临时下载与清理。
  - `ls` / `md` / `rm`：远端目录清单、递归创建目录以及非空删除安全拦截（脚本传 `-y` 可跳过确认）。
...
| 命令 | 行为与特性 |
| :--- | :--- |
| `cw cp [-r] [-j <n>] <src> <dest>` | 支持文件与目录 本地 $\leftrightarrow$ 远端、远端 $\leftrightarrow$ 远端；单遍流式 SHA-256 校验；`-r` 递归拷贝，`-j` 并发连接数（默认 8） |
| `cw cat <node>:<path>` | 直接在本地终端打印远端文件文本内容 |
| `cw ls <node>:<path>` | 结构化输出远端目录结构、大小与修改时间 |
| `cw md <node>:<path>` | 远端递归创建目录（等同 `mkdir -p`） |
