...
| **List Jobs** | `cw ps` | Lists jobs across cluster (`RUNNING`, `COMPLETED`, `FAILED`, `STOPPED`) |
...
| **Copy File/Dir** | `cw cp [-r] [-j <n>] <src> <dest>` | Supports `local <-> remote` and `remote <-> remote` (auto mkdir, `-r` recursive, `-j` concurrency) |
| **Read Remote** | `cw cat <node>:<path>` | Prints remote text file directly to stdout |
| **List Dir** | `cw ls <node>:<path>` | Lists directory entries, mode, size, and mod time |
| **Make Dir** | `cw md <node>:<path>` | Recursive remote directory creation (`mkdir -p`) |
| **Remove** | `cw rm -r -y <node>:<path>` | Non-interactive recursive deletion |
